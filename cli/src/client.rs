...
pub fn send_input(socket_name: &str, task_id: &str, input: &str) -> Result<String, std::io::Error> {
    let cmd = core::ipc::Command::SendInput {
        task_id: task_id.to_string(),
        input: input.to_string(),
    };
    let cmd_str = format!("{}\n", serde_json::to_string(&cmd).unwrap());
    send_command(socket_name, &cmd_str)
}

pub fn subscribe(
    socket_name: &str,
) -> Result<impl Iterator<Item = Result<core::events::Event, std::io::Error>>, std::io::Error> {
    let name = socket_name.to_ns_name::<GenericNamespaced>()?;
    let mut conn = Stream::connect(name)?;
...
mod tests {
    ...
    fn test_command_serialization() {
        ...
        let create_cmd = Command::CreateTask {
            yaml: "test".to_string(),
        };
        ...
        let input_cmd = Command::SendInput {
            task_id: "t1".to_string(),
            input: "hello".to_string(),
        };
        let input_str = serde_json::to_string(&input_cmd).unwrap();
        assert_eq!(
            input_str,
            "{\"SendInput\":{\"task_id\":\"t1\",\"input\":\"hello\"}}"
        );
    }
}
